...
const form = useForm({
    name: props.subuser.name,
    email: props.subuser.email,
    phone: props.subuser.phone,
    role: props.subuser.role,
    is_active: props.subuser.is_active,
});

const submit = () => {
    form.patch(`/subusers/${props.subuser.id}`, {
        onFinish: () => {
            // Nothing to reset on success
        },
    });
};
